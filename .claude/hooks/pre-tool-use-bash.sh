...
# --- main への移動と変更操作を同一コマンドに束ねる回避経路の禁止 ---
# COMMITTABLE は「コマンド送信時のブランチ」で決まる。したがって作業ブランチから
#   git checkout main && git push
# を1コマンドで投げると、判定時は作業ブランチのままなので COMMITTABLE=1 となり素通りする。
# 素の git push は PUSH_ARGS が空で main/force/refspec のどれにも当たらず、upstream の
# main へ push されてしまう（絶対禁止 §1-1 を破れる）。
# 移動そのものは工程9b（/integrate Phase B）に必要なので許可し、束ねることだけを禁じる。
if [[ "$CMD" =~ git[[:space:]]+(checkout|switch)([[:space:]]|$) ]] \
&& [[ "$CMD" =~ (^|[[:space:]])(main|master)([[:space:];\&\|]|$) ]] \
&& [[ "$CMD" =~ (^|[[:space:];&|])git[[:space:]]+(commit|push|merge)([[:space:]]|$) ]]; then
  deny "BLOCKED: main への移動と commit/push/merge を1コマンドに束ねている（CLAUDE.md §1-1）。
ブランチ判定は送信時に行うため、移動を先に挟むとガードを欺ける。
移動と変更は別々のコマンドに分けること（移動後は main 上と判定され、正しくブロックされる）。"
fi

# --- 破壊的 git ---
# checkout/switch の -f / --force / --discard-changes は未コミットの作業を黙って捨てる。
# 対象ブランチを問わず破壊的なのでここで止める（git checkout . と同じ class）。
