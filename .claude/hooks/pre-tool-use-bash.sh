...
# --- DB マイグレーション / スキーマ変更 ---
# staff-report の実行系: prisma migrate（pnpm db:migrate / db:deploy、make db-migrate / db-reset）
if [[ "$CMD" =~ (prisma[[:space:]]+migrate|typeorm[[:space:]]+migration|knex[[:space:]]+migrate|run[[:space:]]+migrate|flyway|liquibase) ]] \
|| [[ "$CMD" =~ db:(migrate|deploy|push) ]] \
|| [[ "$CMD" =~ make[[:space:]]+(db-migrate|db-reset|db-create-local|setup|setup-local)([[:space:]]|$) ]] \
...
（クライアント生成だけなら db:generate は許可されている。）"
fi

# --- main ブランチ上での作業（高速フェイル。正本は GitHub ブランチ保護） ---
if [[ "$CMD" =~ git[[:space:]]+(checkout|switch)[[:space:]]+(main|master)([[:space:]]|$) ]]; then
  deny "BLOCKED: main への checkout。作業は feature/slice-NN / spec/slice-NN のみ（CLAUDE.md §1）。"
fi
...
バグ再現も合成データで作ること（CLAUDE.md §1-6）。"
fi

# --- 権限バイパス（暴走対策の第一則） ---
if [[ "$CMD" =~ bypassPermissions ]] || [[ "$CMD" =~ --dangerously-skip-permissions ]]; then
  deny "BLOCKED: permission バイパスは全ロールで禁止（CLAUDE.md §1-8）。"
fi
