...
RC="$(run_hook protect-paths.sh "$REPO_BOOT" '{"tool_input":{"file_path":"'"$REPO_BOOT"'/apps/service/src/x.ts"}}')"
[[ "$RC" == "2" ]] \
...
RC="$(run_hook protect-paths.sh "$REPO_OK" '{"tool_input":{"file_path":"'"$REPO_OK"'/apps/service/src/x.ts"}}')"
[[ "$RC" == "0" ]] \
  && pass "feature/slice-01 での apps/service 書込は allow (exit 0)" \
  || fail "feature/slice-01 での apps/service 書込が誤ブロック (exit $RC)"
RC="$(run_hook protect-paths.sh "$REPO_OK" '{"tool_input":{"file_path":"'"$REPO_OK"'/apps/service/src/__tests__/integration/x/x.integration.test.ts"}}')"
[[ "$RC" == "2" ]] \
  && pass "feature/slice-01 での 受け入れテスト書込は block (exit 2)" \
  || fail "feature/slice-01 での 受け入れテスト書込が素通り (exit $RC)"
