...
mkrepo() {
  ...
  mkdir -p "$dir/docs/spec" "$dir/apps/service/src/__tests__/integration/reports"
  printf '%s' "$dir"
}
...
# 受け入れテストへの書込を試みる payload
accept_payload() { echo '{"tool_input":{"file_path":"'"$1"'/apps/service/src/__tests__/integration/reports/create.integration.test.ts"}}'; }
...
[[ "$RC" == "0" ]] && pass "approved+画面要件記入済み → 受け入れテスト allow (exit 0)" \
...
RC="$(run_hook protect-paths.sh "$R" "$(accept_payload "$R")")"
[[ "$RC" == "0" ]] && pass "approved+「画面なし」明記 → 受け入れテスト allow (exit 0)" \
