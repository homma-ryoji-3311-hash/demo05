...
- 実装（`apps/service/` `apps/web/`）と `acceptance/` に触らない。
...
| 2. 受け入れテスト | PM＋AIアーキ | `acceptance/` の実パス（**`*.api.spec.ts` と `*.ui.spec.ts` の二層とも**・ADR-0018）・golden 名・仕様表パスを列挙 |
...
- 画面があるスライス → 工程4（`/spec` Phase B）に戻って `ui.spec.ts` を書く。
