...
2. **PR を作成する**
   ...
   - **PR タイトルはこの repo の規約に従う**（CI の lint-pr / commitlint が検証する）:
     `<type>(<scope>): <日本語を含む要約> [SRP-<番号>]`
     - scope は `web` / `service` / `project`。課題キー `[SRP-N]` は**末尾**（先頭や `(SRP-N)` は弾かれる）。
     - 例: `feat(service): 業務報告の保存 API を追加 [SRP-12]`
   - 本文には `/verify` の4判定結果（ADR-0018）と、`/implement` の証拠（テスト生出力）を貼る。
     PR テンプレートの「関連課題」欄に Jira 課題キーを書く。
