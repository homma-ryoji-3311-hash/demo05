...
- **PR タイトルに課題キーを必須にしている**（CI の `lint-pr` で強制）: subject の**末尾に `[SRP-<番号>]`**（例: `feat(web): greeting 画面を追加 [SRP-2]`）。無い／先頭に置く／括弧違い（`(SRP-2)`）は弾かれる。squash マージ時に PR タイトルがコミットメッセージになるため、ここで担保する。
- **個々のコミットメッセージへの課題キーは任意**（commitlint では強制しない。付けても弾かれない）。
- これにより該当 Jira 課題の「開発」欄に PR が自動表示される。
- ブランチ名への課題キーも任意（入れると紐づきがより確実になる）。
