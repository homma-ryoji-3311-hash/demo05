...
- 形式: `<type>/<短い説明>`（例: `feature/greeting-screen`, `fix/auth-401`）。
  ...
  - 課題キーの付与は任意（Jira の「ブランチを作成」から作ると自動で `KAN-<番号>` 入りになる。入れておくと連携がより確実）。

## Jira 連携

タスク管理は Jira Cloud（プロジェクトキー `KAN`）で行う。GitHub とは「GitHub for Jira」で連携している。

- **コミット / PR タイトルに課題キーを必須にしている**（commitlint・CI で強制）: subject の**末尾に `[KAN-<番号>]`**（例: `feat(web): greeting 画面を追加 [KAN-2]`）。無い／先頭に置く／括弧違い（`(KAN-2)`）は弾かれる。
...
- 関連する Jira 課題キー `KAN-<番号>` を PR タイトルまたは本文に含める（テンプレートの「関連課題」欄）。
