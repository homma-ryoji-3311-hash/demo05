...
/**
 * reports ルーター（slice-01 スコープ: 作成・更新・下書き取得）。
 * router → service(use-case) → repository の一方向を守る。
 * 認証 seam: すべて protected。authUserId(req) が X-User-Id を必須で読む（空/無しは 401）。
 * エラーは throw → next 経由で共通の error-handler が HTTP に変換する（401/404/409/422）。
 * 経路順: 具体パス（/draft）を動的パス（/:id）より前に置く。
 * 一覧(GET /)・詳細(GET /:id)・summarize・confirm・previous は後続スライスで足す。
 */
